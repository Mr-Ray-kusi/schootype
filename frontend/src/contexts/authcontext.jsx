...
const isPaymentRoute = (url = '') =>
  url.includes('/api/school/subscription/') ||
  url.includes('/api/fees/') ||
  url.includes('/api/payments/') ||
  url.includes('/api/wallet') ||
  url.includes('/api/platform/wallet');
...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const interceptor = axios.interceptors.response.use(
      ...
      (error) => {
        ...
        if ((status === 401 || status === 403) && !isAuthRoute(url) && !isPaymentRoute(url)) {
          clearSession();
        }
  ...
  const refreshSchool = async (schoolData) => {
    if (schoolData) {
      persistSchool(schoolData);
      return schoolData;
    }
